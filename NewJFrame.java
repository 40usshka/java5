    ...
    private void jButtonClearActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonClearActionPerformed
        ...
        Table.addTableModelListener((TableModelEvent e) -> {
            if (e.getType() == TableModelEvent.UPDATE) {
                array.get(jTable1.getSelectedRow()).setDataByIndex(jTable1.getSelectedColumn(),
                        (String) Table.getDataVector().get(jTable1.getSelectedRow()).get(jTable1.getSelectedColumn()));
            }
        });
 ...
 */
    class JThread extends Thread{
        ...
        @Override
        public void run(){
            double dx = (up-down)/step;
                for (int i = 1; i < dx; i++){
                    if(!((down+step)>up)) {
                        result+= (Math.cos(down+step)+Math.cos(down))*step/2;
                        down += step;
                    }
                    else result+= (Math.cos(down)+Math.cos(up))*(up-down)/2;
            }
        }
    }
